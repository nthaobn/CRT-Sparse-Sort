...
// 1. Phân rã đơn luồng (Sequential Decomposition)
void decompose_sequential(const std::vector<uint64_t>& input, std::vector<PackedElement>& packed_vec) {
    size_t n = input.size();
...
// 2. Sắp xếp Radix đơn luồng (Sequential Radix Sort)
void radix_sort_sequential(std::vector<PackedElement>& packed_vec) {
    ...
    for (int shift = 0; shift < 64; shift += 8) {
        ...
        // Phân phối vào mảng tạm
        for (size_t i = 0; i < n; ++i) {
            uint8_t byte_val = (packed_vec[i].packed_key >> shift) & 0xFF;
            temp[offset[byte_val]++] = packed_vec[i];
        }
...
// 3. Tái tạo đơn luồng (Sequential Reconstruction)
void reconstruct_sequential(const std::vector<PackedElement>& packed_vec, std::vector<uint64_t>& output) {
    size_t n = packed_vec.size();
    for (size_t i = 0; i < n; ++i) {
...
int main() {
    std::cout << "=== CRT SPARSE SORT (SEQUENTIAL VERSION) ===" << std::endl;
    ...
    decompose_sequential(input, packed_vec);
    radix_sort_sequential(packed_vec);
    reconstruct_sequential(packed_vec, output);

    auto end_time = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> duration = end_time - start_time;

    std::cout << "Thoi gian sap xep don luong: " << duration.count() << " ms" << std::endl;
